...
export class Collapse {
  ...
  public collapse: CollapseFunctionMap = new Map([
    ...
    [
      ...
      (node, complexity) => {
        ...
        if (
          typeof this.cutoff.fenced === "number" &&
          complexity > this.cutoff.fenced &&
          ...
          complexity = this.recordCollapse(
            ...
            this.getText(node.childNodes[0] as MmlNode) +
              this.getText(
                node.childNodes[node.childNodes.length - 1] as MmlNode
              )
          );
        }
        return complexity;
    ...
    [
      ...
      (node, complexity) => {
        ...
        if (
          typeof this.cutoff.sqrt === "number" &&
          complexity > this.cutoff.sqrt
        ) {
          complexity = this.recordCollapse(
            node,
            complexity,
    ...
    [
      ...
      (node, complexity) => {
        if (
          (typeof this.cutoff.bigop === "number" &&
            complexity > this.cutoff.bigop) ||
          !node.isKind("mo")
        ) {
    ...
    [
      ...
      (node, complexity) => {
        if (
          (typeof this.cutoff.integral === "number" &&
            complexity > this.cutoff.integral) ||
          !node.isKind("mo")
        ) {
          const id = this.splitAttribute(node, "content").pop();
          const op = this.findChildText(node, id);
    ...
    [
      ...
      (node, complexity) => {
        if (
          typeof this.cutoff.relseq === "number" &&
          complexity > this.cutoff.relseq
        ) {
    ...
    [
      ...
      (node, complexity) => {
        complexity = this.uncollapseChild(complexity, node, 0, 2);
        if (
          typeof this.cutoff.superscript === "number" &&
          complexity > this.cutoff.superscript
        ) {
          complexity = this.recordCollapse(
            node,
            complexity,
    ...
    [
      ...
      (node, complexity) => {
        complexity = this.uncollapseChild(complexity, node, 0, 2);
        if (
          typeof this.cutoff.subscript === "number" &&
          complexity > this.cutoff.subscript
        ) {
          complexity = this.recordCollapse(
            node,
            complexity,
    ...
    [
      ...
      (node, complexity) => {
        ...
        if (
          typeof this.cutoff.subsup === "number" &&
          complexity > this.cutoff.subsup
        ) {
          complexity = this.recordCollapse(
            node,
            complexity,
  ...
  public check(node: MmlNode, complexity: number): number {
    ...
    if (this.collapse.has(type)) {
      const fn = this.collapse.get(type);
      if (fn) {
        return fn.call(this, node, complexity);
      }
    }
  ...
  protected canUncollapse(
    ...
    if (this.splitAttribute(node, "children").length === m) {
      const mml =
        ...
        (node.childNodes[0] as MmlNode).isInferred
          ? (node.childNodes[0] as MmlNode)
          : node;
  ...
  protected uncollapseChild(
    ...
    if (child) {
      ...
      if (child.parent !== node) {
        child.parent.attributes.set("data-semantic-complexity", null as any);
      }
  ...
  protected getText(node: MmlNode): string {
    ...
    return (node.childNodes as MmlNode[]).map((n) => this.getText(n)).join("");
  }
   ...
   */
  protected findChildText(node: MmlNode): string {
    return (node.childNodes as MmlNode[]).map((n) => this.getText(n)).join("");
  }
  ...
  public makeCollapse(node: MmlNode) {
    const nodes: MmlNode[] = [];
    node.walkTree((child) => {
      const mml = child as MmlNode;
      if (mml.getProperty("collapse-marker")) {
        nodes.push(mml);
      }
    });
  ...
  public addMrow(node: MmlNode): MmlNode {
    const mrow = this.complexity.factory.create(
      "mrow",
      undefined,
      node.childNodes[0].childNodes as MmlNode[]
    );
